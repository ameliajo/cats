...
def simpleGASKET(rhoBetas,rho,time,alphas,betas,flagUnstable = False):
    ...
    for a,alpha in enumerate(alphas):
        ...
        for b,beta in enumerate(betas):
            ...
            if not unstable:
                unstable = areWeUnstable(a,b,betas,sab,alpha_H_exp,cos_alpha_F,\
                                         sin_alpha_F,time,alpha_exp)
                if unstable == True:
                    for i in range(-5,0):
                        sab_approx = getSAB_T1_approx(rhoBetas,rho,[alphas[a]],\
                                     [betas[b+i]])[0]*exp(-betas[b])
                        plt.plot(betas[b+i],sab_approx,'yo')
 
            if unstable:
                plt.plot(beta,sab[b+a*len(betas)],'ro')
                sab_approx = getSAB_T1_approx(rhoBetas,rho,[alphas[a]],\
                             [betas[b]])[0]*exp(-betas[b])
...
if __name__=="__main__":
    ...
    time = np.linspace(0,100,1e3)

    sab,H,F = simpleGASKET(uniform_x[1:],uniform_y[1:],time,alphas,betas)
    ...
    for a in range(len(alphas)):
        ...
        sab_nonsym_neg_side = getSAB_phononExpansion(5,uniform_x[1]-uniform_x[0],\
                              uniform_y,[alphas[a]],betas)
        print("Just finished phonon distribution")
        sab_nonsym_pos_side = [sab_nonsym_neg_side[b]*exp(-betas[b]) \
