...
for a in range(0,len(alphas)):
    ...
    gasket_sym = [0.0255*gasket_sab[b+a*len(betas)] for b in range(len(betas))]
    plt.plot(betas,njoy_sym,color=scalarMap.to_rgba(a),linewidth=2)
    if gasket_sym[-1] < 1e-7:
        plt.plot(betas[:-1],gasket_sym[:-1],color=scalarMap.to_rgba(a),marker='o',markersize=3,linewidth=0)
    else:
        plt.plot(betas,gasket_sym,color=scalarMap.to_rgba(a),marker='o',markersize=3,linewidth=0)
