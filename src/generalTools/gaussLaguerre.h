...
auto gaussLaguerre( int order, double a, double b )
...
//    John Burkardt
{
  ...
  std::vector<double> wVec (order,0.0);
  std::vector<double> xVec (order,0.0);
...
//
{
  ...
  if ( kind == 1 ) {
    ...
    for ( i = 0; i < m; i++ ) { aj[i] = 0.0; }
    for ( i = 1; i <= m; i++ ){ bj[i-1] = sqrt((i + alpha * (i%2))/2.0); }
  }
  else if ( kind == 7 ) {
    ab = alpha;
    zemu = 2.0 / ( ab + 1.0 );
